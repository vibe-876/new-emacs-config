...
We then start the Emacs Server.
#+begin_src elisp
  (server-start)
#+end_src
...
I've recently moved from [[https://github.com/skeeto/elfeed][elfeed]] to newsticker, and I'm quite happy with it.
...
We also want common lisp, we <3 all lisps equally.
#+begin_src elisp
  (use-package sly
    :config
...
For ~sh-mode~, this is just a quick bit of Elisp that'll make the file executable upon saving.
#+begin_src elisp
